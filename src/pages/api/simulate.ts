...
async function run(input: SimulationInput, locals: App.Locals): Promise<Response> {
  ...
  const corridorProviderIds = PROVIDERS.filter((p) =>
    p.supportedCurrencies.includes(input.targetCurrency)
  ).map((p) => p.id);
  const { byProvider, fetchErrors, sourceStatus } = await resolveObservedRates(
    corridorProviderIds,
    kv,
    {
